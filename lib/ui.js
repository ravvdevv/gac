...
export async function createAIDiscoveryFiles(repoRoot) {
  const instructions = `# AI Development Instructions

This project uses **gac** (Git Auto Commit) to maintain high-quality, conventional commit history.

## Commit Workflow
When you have changes to commit, **do not** use standard \`git commit\` commands. Instead, use the \`gac\` CLI:
...
### Dry Run (Review message before commit)
\`\`\`bash
gac --dry-run --json
\`\`\`
...
- **AI-Native**: Always use \`gac\` to ensure consistency with the project's preferred AI model and truncation rules.
`;

  const files = [
    { name: 'ai-instructions.md', content: instructions },
    { name: '.cursorrules', content: 'Please follow the instructions in ai-instructions.md for git commits.' },
    { name: '.windsurfrules', content: 'Please follow the instructions in ai-instructions.md for git commits.' },
    { name: '.aiannotations', content: 'Please follow the instructions in ai-instructions.md for git commits.' },
  ];
