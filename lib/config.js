...
const configWithEnv = {
  get(key) {
    ...
    if (key === 'fallbackModel' && process.env.GAC_FALLBACK_MODEL) {
      return process.env.GAC_FALLBACK_MODEL;
    }
  ...
  get store() {
    ...
    if (process.env.GAC_FALLBACK_MODEL) store.fallbackModel = process.env.GAC_FALLBACK_MODEL;
    return store;
  }
};
