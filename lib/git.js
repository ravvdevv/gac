...
export async function unstageAll() {
  try {
    ...
    // Fine on first commit or detached HEAD
  }
}

function getFileStatusLabel(f) {
  const wd = f.working_dir;
  const idx = f.index;
  if (wd === '?') return 'untracked';
  if (wd === 'M') return 'modified';
  if (wd === 'D') return 'deleted';
  if (wd === 'A') return 'added';
  if (wd === 'R') return 'renamed';
  if (wd === 'U') return 'unmerged';
  if (idx && idx !== ' ' && idx !== '?' && idx !== '!' && wd === ' ') return 'staged';
  return 'changed';
}
...
export async function getAllChangedFiles() {
  ...
  return (status.files || []).map(f => {
    const inIndex = f.index && f.index !== ' ' && f.index !== '?' && f.index !== '!';
    return { path: f.path, staged: inIndex, label: getFileStatusLabel(f) };
  });
}

export async function getStagedStats() {
  const out = await git.raw(['diff', '--cached', '--numstat']).catch(() => '');
